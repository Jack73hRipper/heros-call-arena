...
/**
 * ActionBar — Unified horizontal action + skill bar (top of screen).
 * Phase 6E-4: Merges skills + potions + queue controls into a single bar.
 *
 ...
 */
export default function ActionBar({ onAction, onLeave }) {
  const {
  ...
  const handleSkillClick = useCallback((skill) => {
    ...
    // Placement skills (totems) need uninterrupted tile-selection — clear any
    // active auto-target so pursuit actions don't pre-empt the placement.
    if (isPlacementSkill(skill) && autoTargetId) {
      dispatch({ type: 'CLEAR_AUTO_TARGET' });
      const clearMsg = { type: 'clear_auto_target' };
    ...
    if (actionMode === skillMode) {
      ...
      isPlacementSkill, findNearestTarget, autoTargetId]);
  ...
  const handleSlotMouseEnter = useCallback((skillId, e) => {
  ...
  const handlePotionMouseEnter = useCallback((e) => {
    clearTimeout(tooltipTimerRef.current);
    tooltipTimerRef.current = setTimeout(() => {
  ...
  return (
    <div className="action-bar">
      ...
      <div className="action-slots">
        ...
        {skills && skills.length > 0 && skills.map((skill, index) => {
          ...
          return (
            <div
              ...
              onMouseEnter={(e) => handleSlotMouseEnter(skill.skill_id, e)}
        ...
        <div
          className="action-slot-wrapper"
          onMouseEnter={(e) => handlePotionMouseEnter(e)}
          onMouseLeave={handleSlotMouseLeave}
        >
          <button
      ...
      {/* ---- Skill/Potion Tooltip Panel ---- */}
      {(() => {
        // Find hovered skill
        const hoveredSkill = hoveredSkillId
          ? (skills || []).find(s => s.skill_id === hoveredSkillId)
          : null;
        const hoveredIndex = hoveredSkill
          ? (skills || []).indexOf(hoveredSkill)
          : -1;
        const cd = hoveredSkill ? (activeUnit?.cooldowns?.[hoveredSkill.skill_id] || 0) : 0;

        if (hoveredSkill) {
          return (
            <div className="action-bar-tooltip-panel">
              <SkillTooltip
                skill={hoveredSkill}
                hotkey={String(hoveredIndex + 1)}
                cooldown={cd}
                isAutoAttack={hoveredSkill.is_auto_attack}
                attackerStats={{
