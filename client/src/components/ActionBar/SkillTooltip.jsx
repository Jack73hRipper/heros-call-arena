...
function formatEffect(effect) {
  switch (effect.type) {
    ...
    case 'magic_damage': {
      const mult = effect.damage_multiplier || 1.0;
      return `${Math.round(mult * 100)}% magic damage`;
    }
    case 'aoe_magic_damage': {
      const mult = effect.damage_multiplier || 1.0;
      return `${Math.round(mult * 100)}% magic damage to all enemies within ${effect.radius} tile(s) of target`;
    }
    ...
    case 'lifesteal_aoe': {
      const mult = effect.damage_multiplier || 1.0;
      const healPct = Math.round((effect.heal_pct || 0) * 100);
      return `${Math.round(mult * 100)}% damage to all enemies within ${effect.radius} tile(s), heal ${healPct}% of total damage dealt`;
    }
    ...
    case 'aoe_hot':
      return `Heal allies within ${effect.radius} tiles for ${effect.heal_per_turn} HP/turn (${effect.duration_turns} turns)`;
    case 'damage_absorb':
    ...
    case 'ranged_root': {
      const dur = effect.root_duration || 1;
      const empDur = effect.empowered_root_duration || dur;
      const threshPct = Math.round((effect.empowered_hp_threshold || 0.5) * 100);
      return `Root enemy for ${dur} turn(s). Below ${threshPct}% HP: ${empDur} turn(s)`;
    }
    case 'deaths_embrace_buff': {
      const thorns = effect.thorns_damage || 0;
      const armor = effect.armor_bonus || 0;
      const heal = effect.heal_on_hit_taken || 0;
      const dur = effect.duration_turns || 0;
      return `Reflect ${thorns} dmg/hit, +${armor} armor, heal ${heal} HP when hit (${dur} turns)`;
    }
    case 'melee_damage_conditional_bleed': {
      const mult = effect.damage_multiplier || 1.0;
      const empMult = effect.empowered_damage_multiplier || mult;
      const threshPct = Math.round((effect.empowered_hp_threshold || 0.5) * 100);
      const bleedDmg = effect.bleed_damage_per_turn || 0;
      const bleedDur = effect.bleed_duration || 0;
      return `${Math.round(mult * 100)}% melee damage. Below ${threshPct}% HP: ${Math.round(empMult * 100)}% + bleed ${bleedDmg}/turn (${bleedDur} turns)`;
    }
    case 'undying_fury': {
      const window = effect.activation_window || 5;
      const revivePct = Math.round((effect.revive_hp_pct || 0.25) * 100);
      const furyDur = effect.fury_duration || 3;
      const furyDmg = Math.round(((effect.fury_damage_multiplier || 1.5) - 1) * 100);
      const furyHeal = Math.round((effect.fury_lifesteal_pct || 0.25) * 100);
      return `${window}-turn window. On death: revive at ${revivePct}% HP, ${furyDur}-turn Fury (+${furyDmg}% damage, CC immune, ${furyHeal}% lifesteal)`;
    }
    default:
      return effect.type;
  }
...
function computeDamageEstimate(effect, attackerStats) {
  ...
  switch (effect.type) {
    ...
    case 'melee_damage_conditional_bleed': {
      ...
      const mult = effect.damage_multiplier || 1.0;
      const empMult = effect.empowered_damage_multiplier || mult;
      const bleedTotal = (effect.bleed_damage_per_turn || 0) * (effect.bleed_duration || 0);
      const normalDmg = Math.round(attack_damage * mult);
      const empDmg = Math.round(attack_damage * empMult);
      return `~${normalDmg} normal, ~${empDmg} + ${bleedTotal} bleed empowered`;
    }
    default:
      return null;
  }
